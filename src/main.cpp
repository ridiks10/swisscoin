...
// tutaj
int64 nChainStartTime = 1399764306;
...
// added because compiler wanted
	...
	if (nHeight < 7200) return (100 * COIN);
	if (nHeight < 28800) return (1000 * min(nSubsidy, MAX_MINT_PROOF_OF_WORK));
	if (nHeight = 28800) return (1000 * COIN);
	else return min(nSubsidy, MAX_MINT_PROOF_OF_WORK);
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SwissCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis Block:
        const char* pszTimestamp = "Good time for investment is NOW!";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1399764306;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 3619679;

        //// debug print
        printf("block.GetHash() == %s\n", block.GetHash().ToString().c_str());
        printf("block.hashMerkleRoot == %s\n", block.hashMerkleRoot.ToString().c_str());
        assert(block.hashMerkleRoot == uint256("0x824b527f9fabbf60238a16942de32d71f298e45df66247ba4a657839dbb7ec16"));
// gen
	    uint256 hash = block.GetHash();
        // set to true to mine genesis block - otherwise false
